...
#-------------------------------------------------------------------------------------
    ...
    def UpdatePortChannels(self):
        for device_name in sorted(portchannel_list):
            for interface in sorted(portchannel_list[device_name]):
                ...
                pc_profile = portchannel_list[device_name][interface]["Port-Channel Profile"]
                ...
                if pc_profile:
                    interface_list[device_name][interface]["Profile 1"] = pc_profile
    ...
    def GetProfileList(self):
        ...
        for row_no in range(len(raw_db["profiles"])):
            ...
            profile = raw_db["profiles"][row_no]["Profile"]
            if not self.GetVariable(profile):
                if not self.GetConfigTemplate(profile):
                    error_db["profiles"].append("Row ({}): Device '{}' referenced variable '{}' which does not exist".format(raw_db["profiles"][row_no]["Row"],device_name,profile))
                    continue
            if device_name not in profile_list:
                temp_list = []
                profile_list[device_name] = temp_list
            profile_list[device_name].append(profile)
    ...
    def has_profile1_configured(self,device,interface):
        if interface_list[device][interface].get("Profile 1"):
            return True

    def has_profile2_configured(self,device,interface):
        if interface_list[device][interface].get("Profile 2"):
            return True
    ...
    def has_mtu_configured(self,device,interface):
        if interface_list[device][interface]["MTU (leave blank for default)"]:
            return True
    ...
    def RemoveEmptyRowsFromDB(self):
        ...
        db_required_columns["profiles"]= ["Device Name","Profile"]
    ...
    def CreateGlobalConfig(self,device_name):
        if not profile_list.get(device_name):
            return
        print ("!---------------------------------")
        print ("! Global configuration ")
        print ("!---------------------------------")
        for number, profile in enumerate(profile_list[device_name]):
            profile_name = profile_list[device_name][number]
    ...
    def CreateVrfConfig(self,device_name):
        ...
        for vrf in sorted(vrf_list[device_name]):
            ...
            if vrf_list[device_name][vrf]["Profile"]:
                if self.is_valid_variable(vrf_list[device_name][vrf]["Profile"]):
                    print (self.GetVariable(vrf_list[device_name][vrf]["Profile"]))
    ...
    def CreateInterfaceConfig(self,device_name,config_mode):
        ...
        for interface in sorted(interface_list[device_name]):
            ...
            if self.has_mtu_configured(device_name,interface):
                print ("  mtu {}".format(interface_list[device_name][interface]["MTU (leave blank for default)"]))
            ...
            if self.has_profile1_configured(device_name,interface):
                if self.GetVariable(interface_list[device_name][interface]["Profile 1"]):
                    print (self.GetVariable(interface_list[device_name][interface]["Profile 1"]))
            if self.has_profile2_configured(device_name,interface):
                if self.GetVariable(interface_list[device_name][interface]["Profile 2"]):
                    print (self.GetVariable(interface_list[device_name][interface]["Profile 2"]))
    ...
    def CheckInterfacesForErrors(self):
        for device in interface_list:
            for interface in sorted(interface_list[device]):
                ...
                if self.has_profile1_configured(device,interface):
                    if not self.GetVariable(interface_list[device][interface]["Profile 1"]):
                        error_db["interfaces"].append("Row ({}): [{}] [{}] referenced variable '{}' which does not exist".format(interface_list[device][interface]["Row"],device,interface,interface_list[device][interface]["Profile 1"]))
                if self.has_profile2_configured(device,interface):
                    if not self.GetVariable(interface_list[device][interface]["Profile 2"]):
                        error_db["interfaces"].append("Row ({}): [{}] [{}] referenced variable '{}' which does not exist".format(interface_list[device][interface]["Row"],device,interface,interface_list[device][interface]["Profile 2"]))
